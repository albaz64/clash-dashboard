...
export default function Settings () {
    ...
    async function handleReloadConfig () {
        try {
            ...
            if (resp.status === 204) {
                success(t('externalControllerSetting.messages.reloadOk') as string)
            } else {
                error(`${t('externalControllerSetting.messages.reloadErr')} ${resp.data}`)
            }
            await fetchGeneral()
        } catch (e) {
            let msg = `${e}`
            ...
            error(`${t('externalControllerSetting.messages.reloadErr')} ${msg}`)
        }
    }
